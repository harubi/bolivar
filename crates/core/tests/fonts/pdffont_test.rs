...
use bolivar_core::pdffont::{FontWidthDict, MockPdfFont, PDFCIDFont, PDFFont, get_widths};
...
fn test_get_cmap_from_spec() {
    ...
    spec.insert("Encoding".into(), PDFObject::Name(cmap_name.into()));
...
fn test_get_cmap_identity_h() {
    let mut spec = PDFDict::default();
    spec.insert("Encoding".into(), PDFObject::Name("Identity-H".into()));
...
fn test_get_cmap_identity_v() {
    let mut spec = PDFDict::default();
    spec.insert("Encoding".into(), PDFObject::Name("Identity-V".into()));
...
fn test_get_cmap_one_byte_identity() {
    ...
    spec.insert(
        "Encoding".into(),
        PDFObject::Name("OneByteIdentityH".into()),
    );
...
fn test_pdffont_char_width_no_widths() {
    let pdffont = MockPdfFont::new(PDFDict::default(), FontWidthDict::default(), 100.0);
...
fn test_pdffont_char_width_int_cid() {
    ...
    let pdffont = MockPdfFont::new(PDFDict::default(), widths, 100.0);
...
fn test_pdffont_char_width_invalid_int() {
    let mut widths: FontWidthDict = FontWidthDict::default();
