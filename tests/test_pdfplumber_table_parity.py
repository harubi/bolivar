...
class TestTableParity(unittest.TestCase):
    ...
    def test_table_settings_errors(self):
        with pytest.raises(ValueError):
            table.TableFinder(self.pdf.pages[0], tuple())
        ...
        with pytest.raises(ValueError):
            table.TableFinder(self.pdf.pages[0], {"vertical_strategy": "x"})
    ...
    def test_rows_and_columns(self):
        ...
        with pdfplumber.open(path) as pdf:
            ...
            row = [page.crop(bbox).extract_text() for bbox in table_obj.rows[0].cells]
            assert row == [
                "Line no",
                "UPC code",
                "Location",
    ...
    def test_text_layout(self):
        ...
        with pdfplumber.open(path) as pdf:
            table_obj = pdf.pages[0].extract_table(
                {
                    "text_layout": True,
                }
            )
            assert table_obj[3][0] == "   FY2013   \n   FY2014   "
    ...
    def test_extract_tables_counts(self):
        path = os.path.join(HERE, "pdfs/issue-336-example.pdf")
        with pdfplumber.open(path) as pdf:
            tables = pdf.pages[0].extract_tables()
            assert len(tables) == 3
